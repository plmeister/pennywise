...
class TransactionService(BaseService[Transaction]):
    ...
    def create_transfer(
        ...
        from_account = self.db.query(Account).get(from_account_id)
        to_account = self.db.query(Account).get(to_account_id)
        ...
        transaction = Transaction(
            ...
            date=transaction_date or datetime.utcnow().date()
        )
    ...
    def create_multi_leg_transaction(
        ...
        if abs(total_debits - total_credits) > 0:
            raise ValueError("Transaction legs must balance - total debits must equal total credits")
    ...
    def get_account_balance(self, account_id: int, as_of_date: date = None) -> Decimal:
    ...
    def get_pot_balance(self, pot_id: int, as_of_date: Optional[date] = None) -> Decimal:
        """
    ...
    def transfer_to_pot(
        ...
        return self.create_multi_leg_transaction(
            ...
            description=description or f"Transfer to pot",
    ...
    def transfer_from_pot(
        ...
        return self.create_multi_leg_transaction(
            ...
            description=description or f"Transfer from pot",
            transaction_date=transaction_date
        )
    ...
    def transfer_between_pots(
        ...
        return self.create_multi_leg_transaction(
            ...
            description=description or f"Transfer between pots",
            transaction_date=transaction_date
        )
